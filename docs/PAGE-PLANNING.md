# MindWP page planning

This guide helps think a page through before it is designed. It is not a workflow, a form or a page catalogue. A simple page that answers everything here has been over-planned.

[FOUNDATION.md](./FOUNDATION.md) and [STRATEGY.md](./STRATEGY.md) own the business. [WRITING.md](./WRITING.md) owns expression. [DESIGN.md](./DESIGN.md) owns visual interpretation and composition. A page plan applies those authorities to one buyer's decision without redefining them.

Page-specific plans are supplied separately, and only when the relevant page is being worked on.
...
A useful plan establishes, in whatever form suits the page:

- **The page's commercial responsibility.** What this page has to achieve for the business, and what it would mean for the page to have failed.
- **The buyer and their decision.** Who arrives, what they are trying to settle, what they already believe, and what they are comparing this against.
- **Search or task intent** where it applies — not a keyword, but what the visitor is trying to understand, compare, verify or do, and what evidence would satisfy that.
- **The desired action**, and any justified lower-commitment path.
- **Offer and sibling-page boundaries.** What this page owns, what it may mention without absorbing, and what belongs to an adjacent page. Strategy sets page roles at business scale; the plan makes them concrete for this page — where the argument stops, and where the next page picks it up.
- **The objections that are real for this buyer**, and where in the progression each one becomes pressing.

Specificity comes from the actual decision, its language, its evidence and its professional boundaries. An industry name applied to generic reasoning is not specificity.
...
Proof is where MindWP pages are most often forced into invention, so plan it explicitly rather than assuming it will appear.

For each important claim or demand, establish which of these applies:

- **Real evidence already exists** — name it, and its permission or anonymity boundary.
- **MindWP can produce a real artefact** — the actual wording, record, structure or configuration the work produces. This is usually the strongest material available and needs nobody's permission.
- **It may be honestly constructed and labelled**, with its status visible wherever a visitor could mistake it for client work or a measured result.
- **It would need to be developed** as future proof, and does not exist yet.
- **It is unavailable**, and must not be simulated.

Then answer the question that actually decides the page: **what does this page do where the ideal proof does not exist?** Usually the answer is quieter material, a narrower claim, or moving the weight to a matter that can be evidenced. It is never a convincing fabrication.

Do not reconstruct proof from memory or from another page. If evidence is not supplied, it is unavailable.
...
## Where the material asks for more than reading

Some material carries a relationship that plain explanation flattens. A plan may note when the material appears to call for comparison, inspection, progression, a change of state, participation, selective detail, evidence at real scale, continuity across several matters, or a deliberate pause.

These are observations about the material, offered so design can consider them. They are not instructions, a list to work through, or a requirement that every page contain any of them.

**The plan must not prescribe the mechanism.** Cards, tabs, accordions, sticky compositions, horizontal or vertical scroll sequences, galleries, interactive models and animation are all design's to choose, reject or replace with something better — unless the owner explicitly fixes one, in which case mark it fixed and say why.

## Wording, headings and supplied copy

A plan may fix wording where meaning, law, search intent or an approved claim depends on it. Mark it and say why.
...
## Receiving a plan made elsewhere

A plan may arrive from the owner, from another conversation, or already carrying design decisions. Treat it as decisions, not as raw material to rediscover.

1. Identify what it settles — argument, matters, demands, proof, wording, experience direction, composition, or a complete specification.
2. Check it against four things only: does it contradict an active authority; does it depend on proof that does not exist; does it silently settle an open commercial decision; does it create a serious design, accessibility or technical problem.
3. Report material gaps and conflicts briefly.
4. Continue from the latest stage it reaches.

**Do not replan, rewrite or reformat a supplied plan to fit this document's shape.** Translating an owner's plan into the vocabulary here is busywork unless something is genuinely missing. A plan that answers none of these headings and still tells a designer what to build is a good plan.

An incomplete plan is usually still usable. Fill what is fillable, say in one line what you filled and on what basis, and stop only where proceeding would require inventing proof, a claim or a commercial term.
...
- Does the page have one intelligible job and one clear action?
- Does each demand name a verb the reader performs, on specific material?
- Could any demand be moved to another MindWP page unchanged?
- Is proof reality settled, including what the page does where the ideal proof is absent?
- Is it clear which matters carry the page?
- Are offer and sibling boundaries explicit?
- Are fixed claims, professional limits and open decisions visible?
- Has anything been fixed that design should still own — a component, a count, a heading that need not be fixed, or a section structure?
- Could a fresh designer produce a genuinely different page from this plan than the last one, and if not, is that because the material really is the same?

Review may revisit the same questions against final copy and renders. Report drift to the authority that owns it rather than creating a second plan or a parallel state system.
