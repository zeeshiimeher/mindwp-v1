...
test("validation rejects malformed or mismatched skill metadata", async () => {
  await withSkillFixture(async (fixtureRoot) => {
    const skillPath = resolve(fixtureRoot, ".claude/skills/mindwp-frontend-quality/SKILL.md");
    await writeFile(
      skillPath,
      "---\nname: wrong_name\ndescription:\nunknown: value\n---\n\n# Broken\n",
    );
    ...
    const metadataPath = resolve(
      fixtureRoot,
      ".claude/skills/mindwp-frontend-quality/agents/openai.yaml",
    );
    await writeFile(
      metadataPath,
    ...
    assert.ok(issues.some((issue) => issue.includes("must mention $mindwp-frontend-quality")));
  });
});

test("validation detects duplicate skill definitions", async () => {
  await withSkillFixture(async (fixtureRoot) => {
    const original = resolve(fixtureRoot, ".claude/skills/mindwp-frontend-quality");
    const duplicate = resolve(fixtureRoot, ".claude/skills/mindwp-frontend-quality-copy");
    await cp(original, duplicate, { recursive: true });

    const issues = await validateSkills(fixtureRoot);
    assert.ok(
      issues.some((issue) => issue.includes("duplicate skill definition 'mindwp-frontend-quality'")),
    );
  });
});
...
test("validation rejects escaping metadata and symlinked skill directories", async () => {
  await withSkillFixture(async (fixtureRoot) => {
    ...
    await symlink(resolve(fixtureRoot, ".claude/skills/mindwp-frontend-quality"), linkedSkill);
...
test("validation requires metadata assets to be files", async () => {
  await withSkillFixture(async (fixtureRoot) => {
    const assetsDirectory = resolve(fixtureRoot, ".claude/skills/mindwp-frontend-quality/assets");
...
test("validation does not enforce doctrine phrases or heading order", async () => {
  await withSkillFixture(async (fixtureRoot) => {
    const skillPath = resolve(fixtureRoot, ".claude/skills/mindwp-frontend-quality/SKILL.md");
    const skill = await readFile(skillPath, "utf8");
    await writeFile(skillPath, skill.replace("## Repair coherently", "## Repair carefully"));
