...
import { focusAndRevealTab } from "@/app/_home-legacy/tabNavigation";
...
export function HomeSystemsSelector() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const progress = flow.querySelector<HTMLElement>(".hl-systems__flow-progress");
        const nodes = gsap.utils.toArray<HTMLElement>(".hl-systems__flow-step", flow);
        ...
        nodes.forEach((node, i) => {
          const dot = node.querySelector(".hl-systems__flow-node");
  ...
  if (isStacked) {
    return (
      <div className="hl-systems__accordion">
        {MOMENTS.map((moment, i) => {
          const open = moment.slug === activeSlug;
          return (
            <article key={moment.slug} className={`hl-systems__row${open ? " is-open" : ""}`}>
              <h3 className="hl-systems__row-heading">
                <button
                  type="button"
                  id={`system-head-${moment.slug}`}
                  className="hl-systems__row-head"
                  aria-expanded={open}
                  aria-controls={`system-body-${moment.slug}`}
                  onClick={() => setActiveSlug(open ? "" : moment.slug)}
                >
                  <span className="hl-systems__row-num" aria-hidden="true">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="hl-systems__row-icon">
                    <Icon name={moment.icon} size={20} />
                  </span>
                  <span className="hl-systems__row-id">{moment.displayName}</span>
                  <span className="hl-systems__row-chevron" aria-hidden="true">
              ...
              <div
                className="hl-systems__row-body"
                id={`system-body-${moment.slug}`}
                role="region"
                aria-labelledby={`system-head-${moment.slug}`}
                inert={!open}
              >
                <div className="hl-systems__row-inner">
                  <div className="hl-systems__statements">
                    <p className="hl-systems__problem">
                      <span aria-hidden="true">×</span>
                      {moment.problem}
                    </p>
                    <p className="hl-systems__promise">
                      <span aria-hidden="true">✓</span>
                      {moment.promise}
                    </p>
                    <p className="hl-systems__earns">{moment.earns}</p>
                    <p className="hl-artifact-label">Part of {moment.tag}</p>
                  </div>

                  <div className="hl-systems__row-flow">
                    <p className="hl-artifact-label">How it works</p>
                    <ol className="hl-systems__row-flow-steps">
                      <span className="hl-systems__row-flow-rail" aria-hidden="true" />
                      <span className="hl-systems__row-flow-progress" aria-hidden="true" />
                      {moment.mechanism.map((step) => (
                        <li key={step.label} className="hl-systems__row-flow-step">
                          <span className="hl-systems__row-flow-node" aria-hidden="true">
                            <Icon name={step.icon} size={16} />
                          </span>
                          <span className="hl-systems__row-flow-copy">
  ...
  return (
    <div className="hl-systems__layout">
      <div
        className="hl-systems__segments"
        role="tablist"
        ...
        {MOMENTS.map((moment, index) => {
          ...
          return (
            <button
              ...
              className={`hl-systems__segment${selected ? " is-active" : ""}`}
              onClick={() => setActiveSlug(moment.slug)}
              onKeyDown={(event) => onKey(event, index)}
            >
              <span className="hl-systems__segment-icon">
      ...
      <article
        ...
        key={activeSlugSafe}
      >
        <div className="hl-systems__stage-narrative">
          <span className="hl-systems__stage-badge">
          ...
          <p className="hl-systems__earns">{detail.earns}</p>
          <p className="hl-artifact-label">Part of {detail.tag}</p>
        </div>

        <div className="hl-systems__stage-flow">
          <p className="hl-artifact-label">How it works</p>
          <ol className="hl-systems__flow" ref={flowRef}>
            <span className="hl-systems__flow-rail" aria-hidden="true" />
            <span className="hl-systems__flow-progress" aria-hidden="true" />
            {detail.mechanism.map((step) => (
              <li key={step.label} className="hl-systems__flow-step">
                <span className="hl-systems__flow-node" aria-hidden="true">
                  <Icon name={step.icon} size={16} />
                </span>
                <div className="hl-systems__flow-copy">
                  <strong>{step.label}</strong>
                  <small>{step.note}</small>
                </div>
              </li>
