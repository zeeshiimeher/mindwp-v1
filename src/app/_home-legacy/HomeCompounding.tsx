...
export function HomeCompounding() {
  ...
  return (
    <section id="compounding" className="hl-compounding section on-mist">
      <div
      ...
      >
        ...
        <p data-hl-sequence-item>{COMPOUND_INTRO}</p>
      </div>

      <div className="container hl-compounding__stage" data-hl-fade>
        <div
          className="hl-compounding__map"
          ref={mapRef}
          role="tablist"
          aria-label="What compounds after launch"
        >
          <svg className="hl-compounding__links" viewBox="0 0 100 100" aria-hidden="true" preserveAspectRatio="none">
            {NODE_POS.map((pos, i) => (
              <line
                key={i}
                x1="50"
                ...
                x2={pos.x}
                y2={pos.y}
                className={`hl-compounding__link${i === index ? " is-active" : ""}`}
              />
            ))}
          </svg>

          <span className="hl-compounding__ring" aria-hidden="true" />

          <span className="hl-compounding__hub" aria-hidden="true">
          ...
          {COMPOUNDERS.map((item, i) => (
            <button
              ...
              className={`hl-compounding__node hl-compounding__node--${i + 1}${i === index ? " is-active" : ""}`}
              onClick={() => select(i)}
              onKeyDown={(event) => onKey(event, i)}
            >
              <span className="hl-compounding__node-dot">
                <Icon name={item.icon} size={22} />
              </span>
              <span className="hl-compounding__node-label">{item.short}</span>
            </button>
          ))}
        </div>

        <article
          className="hl-compounding__detail"
          role="tabpanel"
          ...
          key={active.label}
        >
          <span className="hl-compounding__detail-icon">
            <Icon name={active.icon} size={26} />
          </span>
          <div className="hl-compounding__detail-text">
            <p className="hl-compounding__detail-tag">{active.short}</p>
            <h3>{active.label}</h3>
            <p className="hl-compounding__detail-note">{active.note}</p>
          </div>
        </article>

        <p className="hl-compounding__close">{COMPOUND_CLOSE}</p>
      </div>
    </section>
  );
}
